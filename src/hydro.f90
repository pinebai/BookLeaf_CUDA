...
&      //'2X,a8)') nstep,idtel,idtreg,dt,time,grind,t2,sdt
  ...
  ENDDO l1
    ...
    ein = d_ein
     ...
     deallocate(d_rho)
     deallocate(d_rho05) 
     deallocate(d_eos_param) 
     deallocate(d_zdtnotreg) 
     deallocate(d_zmidlength) 
    
    
     if(size(iellocglob)>0) deallocate(d_iellocglob) 
    ...
    deallocate(d_pre05) 
    deallocate(d_ielreg) 
    deallocate(d_pmeritreg) 
    deallocate(d_elvol) 
    deallocate(d_kappareg)  
    deallocate(d_ndxu) 
    deallocate(d_ndyv) 
    deallocate(d_elmass)  
    deallocate(d_ein05)   
    deallocate(d_ein) 
    deallocate(d_ielmat)  
    deallocate(d_eos_type)  
    deallocate(d_ielsort1)  
